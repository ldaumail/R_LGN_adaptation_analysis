...
pvals <- array(0, c(length(folderfilenames),4))
tvals <- 0
df.KR <- 0
p.KR <- 0
...
if(length(bumps_data$peaks)>0) { 
  
   if(length(filenames$layer[i])>0){ #ignore for the data analysis
   logical_filenames[i] <- TRUE #ignore for the data analysis
   } #ignore for the data analysis
  
peak1<- t(bumps_data$peaks[1,])
peak2<- t(bumps_data$peaks[2,])
peak3<- t(bumps_data$peaks[3,])
peak4<- t(bumps_data$peaks[4,])
col_bumps_data <- t(cbind(peak1,peak2,peak3,peak4))

#create labels
labels <- c("P1","P2","P3", "P4")
bumpnb <- matrix(rep(labels, each =length(bumps_data$peaks[1,])), ncol=4)
t_label <- t(cbind(t(bumpnb[,1]),t(bumpnb[,2]),t(bumpnb[,3]),t(bumpnb[,4])))

#create trial index
trial_idx = t(t(rep(1:length(bumps_data$peaks[1,]), times =4)))
...
filename4 <- paste("C:/Users/daumail/Documents/LGN_data/single_units/inverted_power_channels/good_single_units_data_4bumps_more/new_peak_alignment_anal/lmer_results_peaks/lmer_results_orig_03032020_corrected", ".csv", sep = "")
